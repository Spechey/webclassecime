...
if (isset($_GET["mch"]))
{
		...
		foreach($filters as $filter_name => $filter_values)
		{
			...
			for($i=0;$i<sizeof($filter_values);$i++)
			{
				...
				echo "<li class='".((isset($_GET["filterValue"]) && $_GET["filterValue"] == $filter_values[$i] && isset($_GET["filterName"]) && $_GET["filterName"] == $filter_name)?"active":"")."'><a href='?evs=".$_GET["evs"]."&eps=".$_GET["eps"]."&mch=".$mchs->id."&filterName=".$filter_name."&filterValue=".urlencode($filter_values[$i])."' >".(($filter_values[$i])?$filter_values[$i]:"[-]")."</a></li>";
			}
			echo "</ul></ul>";
		}
		...
		foreach($mchs->getResultatsByCoureurs() as $c)
		{
			...
			foreach($bps as $b => $p)
			{
				...
				if (isset($c->data["BlocsInfos"]["Details"]))
				{
					...
					echo "<td class='cBck-".$cBck->id." action ".$cBck->isValideString()."'><a onclick=\"cBck_update('cBck-".$cBck->id."','".(($cBck->isValide())?"cBckDel":"cBckAdd")."','".$cBck->id."')\">".(($cBck->isValide())?"-":"+")."</a></td>";
				}
				else
				{
					echo "<td>err</td>";
